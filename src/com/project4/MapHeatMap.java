...
public class MapHeatMap extends VizPanel implements TouchEnabled, EventSubscriber {
  ...
  private void gridUpdate() {
    ...
    for(Tweet t: tweets){
      if(map.isVisible(t)){
        x = (int) ((t.getLon()-map.getMinLon())/(map.getMaxLon()-map.getMinLon())*gridSizeX);
        y = (int) ((t.getLat()-map.getMinLat())/(map.getMaxLat()-map.getMinLat())*gridSizeY);
        increaseValue(x,y);
      }
    }
    updateMax();
    ...
    for(int j=0;j<square;j++){
      for(int i=0;i<square;i++){
        ...
        if(finX>0 && finX<gridSizeX && finY>0 && finY<gridSizeY){
          ...
          grid[finY][finX] +=Math.floor(square/2) - incr;
        }
      }
    }
  }
